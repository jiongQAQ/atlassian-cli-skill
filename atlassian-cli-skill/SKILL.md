...
description: Use the local `atlassian-cli` command to read, search, create, and update Confluence pages without starting an MCP server. Trigger this skill when the task is to operate Confluence pages or to sync a local Markdown file into Confluence through the terminal, especially when credentials are already provided by `~/.atlassian-cli.env`.
---
...
1. Start with `scripts/run_atlassian_cli.sh ...` for direct Confluence operations.
...
5. Read back the target page after a write operation when correctness matters.
...
- After create or update operations, read the target page back and verify the critical fields.
- Keep secrets in `~/.atlassian-cli.env`; do not embed tokens or internal URLs into the skill files.

## Resources

- `assets/.atlassian-cli.env.example`: Copy this file to `~/.atlassian-cli.env` and fill in the user's real Confluence credentials.
- `scripts/ensure_atlassian_cli.sh`: Install `atlassian-cli` automatically through `uv` when the command is missing.
- `scripts/run_atlassian_cli.sh`: Load `~/.atlassian-cli.env` if present, reject Jira commands, then execute `atlassian-cli`.
